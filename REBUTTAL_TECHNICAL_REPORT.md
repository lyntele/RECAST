...
This report provides additional evidence for the issues shared by the three reviewers: baseline coverage, real query behavior, expensive distance functions, total cost, parameter settings, and the separate net contributions of the cost signal and shadow validation, including the settings under which each gating layer is unnecessary.
...
| Remove cost-signal gate | Shadow validation used | ±0.05% | −12.02% | The gate does not improve pruning; its filtering and signal upkeep only add time |
...
### Net contribution of the cost-signal gate

The cost-signal gate was designed to prune redundant candidate partitions before they consume validation effort: only candidates whose region shows a sustained cost increase enter shadow validation. The controlled comparison shows that this pruning is unnecessary under the evaluated datasets and settings. With shadow validation held fixed, removing the gate changes DC by only 0.05 percent, while mean query time drops by 12.02 percent. The gate also rejects very few candidates in the first place: the two modes without shadow validation perform 1,294,145 (signal-gated) and 1,315,123 (ungated) immediate partitions, so the gate removes only about 1.6 percent of the candidates that satisfy the basic size and evidence conditions.

These two facts have one consistent reading. Under the evaluated datasets and settings, the candidates that pass the basic conditions are largely genuine: the index reaches the same pruning quality whether or not the gate filters them (DC within 0.05 percent), because shadow validation alone stops the low-value ones. The redundancy that the gate was designed to remove is already removed one step later, so the gate contributes no additional pruning and only adds per-query signal maintenance and filtering cost. Removing the gate therefore makes RECAST faster on average across the 20 settings.

Based on this ablation, we will simplify the design in the final version: shadow validation alone decides whether a candidate partition is committed, and the cost-signal gate is removed from the partition path. The underlying statistics remain in use where they do measurable work, namely pivot admission and signal-driven eviction (see the pivot retention results below). The gate would be worth re-enabling only in workloads that generate candidate partitions far more frequently than any evaluated here, so that filtering before validation saves more than its upkeep costs.
...
The two modes without shadow validation perform 1,294,145 and 1,315,123 immediate partitions. The closeness of the two counts confirms the gate analysis above: the cost-signal gate rejects only about 1.6 percent of candidate partitions, so the filtering that matters is performed by shadow validation. The pivot budget of 32 limits the columns stored in one region, and the minimum region size of 512 prevents a small region from being partitioned further. Neither parameter limits the total number of regions, tree depth, or cumulative routing work. Immediate partitioning is slow because many partitions that satisfy the local conditions but save little work accumulate in the tree.
...
Pivot retention decides which pivot columns remain in a region. When their pruning power differs substantially, retaining columns by cumulative gain is more useful than FIFO or LRU. When distance concentration makes all pivot bounds weak and similar, replacement order matters less. Shadow validation decides whether to partition a region at the current point in the query stream. Its role grows with the number of objects affected by one partition and with the number of later queries that use the resulting structure. Conversely, when the data and transition are simple enough that the triggered candidates are mostly genuine, gating becomes unnecessary and can itself cost performance: in the Colors jump and drift runs, immediate partitioning uses 0.91 and 0.84 times the DC of RECAST-full, because holding a good split back for the probing queries costs more than the few bad splits the validation would prevent. The value of any gating layer therefore depends on how many low-value candidates the workload generates. On these datasets that number is large enough for shadow validation to pay off in 17 of 20 settings, but never large enough for the earlier cost-signal gate to pay off: the E145 ablation shows that the gate contributes no measurable DC reduction on its own and that its upkeep adds query time, so we remove it from the partition path as described above. The residual child stores objects whose distance to the split center was not computed. When later queries visit a new region, these objects remain in a separate region that can continue to learn, rather than being repeatedly scanned in the old parent region.
